...
:: Cek file wajib
if not exist loop_bg.mp4 (echo File loop_bg.mp4 tidak ada! && pause && exit)
...
ffmpeg -re -stream_loop -1 -i loop_bg.mp4 -i input_audio.mp3 -i logo.png -filter_complex "[0:v]scale=1280:720,boxblur=10:5[bg];[2:v]scale=100:-1[logo_s];[1:a]showwaves=s=1200x120:mode=line:colors=cyan[wave];[bg][logo_s]overlay=W-w-10:10[bg_l];[bg_l][wave]overlay=(W-w)/2:H-h-50[v_pre];[v_pre]%track_filters%[outv]" -map "[outv]" -map 1:a -c:v libx264 -preset fast -crf 20 -c:a aac -b:a 192k -shortest output_fix.mp4 -y

echo Selesai! Cek file: output_fix.mp4
pause

f"[2:v]scale=100:-1[logo_scaled];[v_elements][logo_scaled]overlay=W-w-10:10,