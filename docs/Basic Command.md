...
```
ffmpeg -loop 1 -i wa.jpeg -i MawarBodas.mp3 -filter_complex "[1:a]showwaves=s=1280x200:mode=cline:colors=white:rate=25[sw];  [sw]format=rgba,colorchannelmixer=aa=0.8[v]; [0:v][v]overlay=x=0:y=H-h:eval=init[outv]" -map "[outv]" -map 1:a -c:v libx264 -pix_fmt yuv420p -c:a copy -shortest output.mp4
```
...
## Circular Wave (Note escaped commas)
```
...
ffmpeg -i audio.mp3 -filter_complex \
"[0:a]avectorscope=s=480x480:zoom=1.5[v]" \
-map "[v]" -map 0:a -c:v libx264 output_vector.mp4
```
---
...
"color=c=black:s=1280x720:r=30[bg]; ^
 [0:a]avectorscope=s=720x720:m=circular:colors=white:zoom=1.5,format=rgba[sw]; ^
 [bg][sw]overlay=x=(W-w)/2:y=(H-h)/2:shortest=1,format=yuv420p[outv]" ^
-map "[outv]" -map 0:a -c:v libx264 -c:a copy -shortest output_black.mp4
```
...
"color=c=black:s=1280x720:r=30[bg]; ^
 ...
 [bg][wave]overlay=y=H-h:shortest=1,format=yuv420p[outv]" ^
-map "[outv]" -map 0:a -c:v libx264 -c:a copy -shortest output_black.mp4
```
...
ffmpeg -i audio.mp3 -filter_complex ^
"color=c=black:s=1280x720:r=30[bg]; ^
 [0:a]showspectrum=s=1280x400:mode=combined[v]; ^
 [bg][v]overlay=y=H-h:shortest=1,format=yuv420p[outv]" ^
-map "[outv]" -map 0:a -c:v libx264 -c:a copy -shortest output_black.mp4
```
```
ffmpeg -i [AUDIO_FILE] -filter_complex \
"color=c=black:s=[WIDTH]x[HEIGHT]:r=30[bg]; \
 [0:a][WAVEFORM_FILTER] [wave]; \
...
```
ffmpeg -loop 1 -i [BACKGROUND_IMAGE] -i [AUDIO_FILE] -filter_complex \
"[1:a]showwaves=s=[WIDTH]x[HEIGHT]:mode=[MODE]:colors=[COLOR1]|[COLOR2][wave]; \
 [0:v]scale=[WIDTH]:[HEIGHT][bg]; [bg][wave]overlay=shortest=1[outv]" \
...
```
ffmpeg -i [AUDIO_FILE] -filter_complex \
"showwaves=s=[WIDTH]x[HEIGHT]:mode=[MODE]:colors=[COLORS],format=yuv420p[v]" \
-map "[v]" -map 0:a -c:v libx264 -c:a copy [OUTPUT].mp4
```
...
1. Add Visual On-Screen Title with Duration
```
...
2. Adding an On-Screen Timer/Timestamp Overlay
